...
from agents.penalty_adapt_ddqn_agent import penalty_adapt_DDQN_agent
...
def main(pilot_name = "laggy_pilot", budget_intervention_rate = 0, n_training_episodes = 1000, train_co_pilot = True, num_models = 1):
    ...
    for i in range(num_models):
        name = pilot_name + "_budget_intervention_rate_" + str(budget_intervention_rate)
        name = name + "_" + str(i)
        PATH = os.path.abspath(os.path.join(os.getcwd(), "../..", "savedModels", name + ".pkl")) 
        #PATH = "savedModels/" + name + ".pkl"
        ...
        co_agent = penalty_adapt_DDQN_agent(
                    device = "cpu", 
                    discount_factor = 0.99, 
                    last_frame = max_timesteps,
                    lr = 1e-3,
        ...
        exp_co_pilot._agent.penalty = checkpoint['penalty']
    ...
    print(mean_1000ep_reward, std_1000ep_reward, mean_1000ep_intervention, std_1000ep_intervention, mean_1000ep_step, std_1000ep_step, mean_1000ep_succ, mean_1000ep_crash)
    print("----------------------------------------------------------")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--budget_intervention_rate', type=float, default=0.3)
