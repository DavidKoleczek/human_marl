...
from easyrl.approximation import Approximation, RLNetwork


class SoftDeterministicDiscretePolicy(Approximation):
    '''
...
class SoftDeterministicPolicyNetwork(RLNetwork):
    def __init__(self, model, space):
        ...
        self._tanh_scale = torch.tensor((1) / 2).to(self.device)
        self._tanh_mean = torch.tensor((1) / 2).to(self.device)
    ...
    def forward(self, state):
        ...
        if self.training:
            return action, log_prob, torch.distributions.Categorical(action).sample()
