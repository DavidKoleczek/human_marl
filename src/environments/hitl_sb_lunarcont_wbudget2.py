...
class HITLSBLunarContBudget(gym.Env):
    """Custom Environment that follows gym interface"""

    def __init__(self, env_name, human, intervention_budget):
    ...
    def step(self, action):
        ...
        if (action[-1] <= 0) or (self.remaining_budget <= 0):
            ...
            action = np.concatenate((action, np.array([-1])))  # -1 is basically adding to the state that a noop was taken
        else:
        ...
        # current action to the observation and the remaining interventions the agent can take
        temp = list(state)
        temp[0] = np.concatenate((state[0], action, np.array([self.remaining_budget])))
        temp[1] -= penalty
        state = tuple(temp)
    ...
    def reset(self):
        ...
        self.state = np.concatenate((self.state, human_action, np.array([-1]), np.array([self.remaining_budget])))
