...
class HITLSBBudgetLunarLanderContEval(gym.Env):
    """Custom Environment that follows gym interface"""

    def __init__(self, env_name, hitl_agent, intervention_budget, do_not_intervene=False):
    ...
    def observation_space(self):
        # add to the size of the state space to accomodate the human's action and intervention constraint (the + 1)
    ...
    def step(self, action):
        ...
        if isinstance(self.hitl_agent, list):
            ...
            if self.env._elapsed_steps > 0:
                ...
                curr_state = np.concatenate((self.state[0][0: -(self.action_space.shape[0] + 1)],
                                             human_action, np.array([self.remaining_budget])))
                # get the HITL agent's action
                hitl_action = hitl_agent.predict(curr_state)[0]
            else:
                # same idea as the if clause
                curr_state = np.concatenate((self.state[0: -(self.action_space.shape[0] + 1)],
                                             human_action, np.array([self.remaining_budget])))
                hitl_action = hitl_agent.predict(curr_state)[0]
        else:
        ...
        if hitl_action[-1] < 0 or self.do_not_intervene:
            ...
            self.remaining_budget -= (1 / self.intervention_budget)
        ...
        temp = list(state)
        temp[0] = np.concatenate((state[0], hitl_action, np.array([self.remaining_budget])))
        state = tuple(temp)
    ...
    def reset(self):
        ...
        # the initial state will not take any action and have a remaining budget of 1
        self.state = np.concatenate((self.state, np.array([0, 0, -1, self.remaining_budget])))
